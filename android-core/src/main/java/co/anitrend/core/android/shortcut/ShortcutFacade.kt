...
class ShortcutFacade(
    ...
    private val shortcutManager by lazy(PUBLICATION) {
        context.systemServiceOf<ShortcutManager>()
    }
    ...
    override fun createShortcuts(vararg shortcuts: Shortcut): Boolean {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N_MR1) {
            val shortcutInfo = shortcuts.mapNotNull { shortcut ->
                ...
                if (intent == null) {
                    Timber.tag(moduleTag).w("Intent for ${shortcut.id} returned null")
                    null
                } else {
    ...
    companion object {
        private val moduleTag = ShortcutFacade::class.java.simpleName
    }
}