...
class TonalPalettes(
    ...
    infix fun accent1(tone: Double): Color =
        accent1.getOrElse(tone) {
            keyColor.transform(tone, style.accent1Spec)
        }

    infix fun accent2(tone: Double): Color =
        accent2.getOrElse(tone) {
            keyColor.transform(tone, style.accent2Spec)
        }

    infix fun accent3(tone: Double): Color =
        accent3.getOrElse(tone) {
            keyColor.transform(tone, style.accent3Spec)
        }

    infix fun neutral1(tone: Double): Color =
        neutral1.getOrElse(tone) {
            keyColor.transform(tone, style.neutral1Spec)
        }

    infix fun neutral2(tone: Double): Color =
        neutral2.getOrElse(tone) {
            keyColor.transform(tone, style.neutral2Spec)
        }

    companion object {
        private val M3TonalValues =
            doubleArrayOf(
                0.0,
                10.0,
                20.0,
        ...
        private val M3SurfaceTonalValues =
            doubleArrayOf(
                0.0,
                4.0,
                6.0,
        ...
        private fun Color.toTonalPalette(tonalValues: DoubleArray = M3TonalValues): TonalPalette =
         ...
         * @see TonalPalettes
         */
        fun ColorScheme.toTonalPalettes(tonalValues: DoubleArray = M3TonalValues): TonalPalettes =
            TonalPalettes(
                keyColor = primary,
