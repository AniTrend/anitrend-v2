...
object DeepLinkRouter : NavigationRouter() {
    ...
    fun forMatchingIntent(uri: String): Intent? {
        Timber.d("DeepLinkRouter for uri: $uri")
        return provider.matchingIntent(uri)
    }
...
object FavouriteTaskRouter : NavigationRouter() {
    ...
    sealed class Param : IParam {
        ...
        @Parcelize
        data class CharacterToggleParam(
            val id: Long,
        ) : Param()

        @Parcelize
        data class StaffToggleParam(
            val id: Long,
        ) : Param()
    }
}
