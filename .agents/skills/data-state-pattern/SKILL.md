...
The read baselines below apply only to non-mutation source shapes. Mutation-only variants are
covered in the mutation-specific pattern and are exempt from the Room-first read contract.
...
- Put the concrete data-layer use-case bridge in the module's `usecase/` package. Simple modules
  that expose a single operation should use a single `XxxUseCaseImpl`. Modules that expose two or
  more distinct operations should use nested `XxxInteractor` classes such as
  `MediaListInteractor`, `ReviewInteractor`, or `FavouriteInteractor`.
- Query sources usually emit `Flow<Model>`, `Flow<List<Model>>`, or `Flow<PagedList<Model>>`;
  mutation sources often emit `Flow<Boolean?>` or a persisted model and then rely on the
  repository wrapper to expose the final `DataState`.
