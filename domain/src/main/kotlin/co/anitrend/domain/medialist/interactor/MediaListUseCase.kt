...
sealed class MediaListUseCase {
    abstract class Sync<State : UiState<*>>(
        ...
        operator fun invoke(param: MediaListParam.Collection): State = repository.sync(param)
    }
    ...
    abstract class GetCollection<State : UiState<*>>(
        ...
        operator fun invoke(param: MediaListParam.Collection): State = repository.getCollection(param)
    }

    abstract class SaveEntry<State : UiState<*>>(
        protected val repository: IMediaListRepository.SaveEntry<State>,
    ) : MediaListUseCase() {
        operator fun invoke(param: MediaListParam.SaveEntry): State = repository.saveEntry(param)
    }

    abstract class SaveEntries<State : UiState<*>>(
        protected val repository: IMediaListRepository.SaveEntries<State>,
    ) : MediaListUseCase() {
        operator fun invoke(param: MediaListParam.SaveEntries): State = repository.saveEntries(param)
    }

    abstract class DeleteCustomList<State : UiState<*>>(
        protected val repository: IMediaListRepository.DeleteCustomList<State>,
    ) : MediaListUseCase() {
        operator fun invoke(param: MediaListParam.DeleteCustomList): State = repository.deleteCustomList(param)
    }

    abstract class DeleteEntry<State : UiState<*>>(
        protected val repository: IMediaListRepository.DeleteEntry<State>,
    ) : MediaListUseCase() {
        operator fun invoke(param: MediaListParam.DeleteEntry): State = repository.deleteEntry(param)
    }
}
