...
sealed class ReviewUseCase {
    ...
    abstract class RateEntry<State : UiState<*>>(
        ...
        operator fun invoke(param: ReviewParam.Rate): State = repository.rate(param)
    }

    abstract class SaveEntry<State : UiState<*>>(
        protected val repository: IReviewRepository.Save<State>,
    ) : ReviewUseCase() {
        operator fun invoke(param: ReviewParam.Save): State = repository.save(param)
    }

    abstract class DeleteEntry<State : UiState<*>>(
        protected val repository: IReviewRepository.Delete<State>,
    ) : ReviewUseCase() {
        operator fun invoke(param: ReviewParam.Delete): State = repository.delete(param)
    }
}
