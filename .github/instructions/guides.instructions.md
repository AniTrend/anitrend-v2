...
- **Android runtime debugging**: prefer Argent-first investigation (device targeting, runtime
  logs, JS/native network evidence, and UI hierarchy capture) before changing serializers,
  mappers, or UI assumptions. Use
  `.agents/skills/android-runtime-investigation/SKILL.md` as the primary workflow. Use ADB
  logcat/Chucker fallback only when Argent evidence is insufficient.
